...
function OnFinish(selProj, selObj)
{
	try
	{
		...
		for (var j = 1; j <= configurations.Count; ++j) {
		    ...
		    if (temp_item.Name == "Debug(static)" && temp_item.PlatformName == "Xbox 360") {
		        temp_item.Activate();
		        break;
		    }
		}
		...
		add_dependency(selProj, get_project("engine/engine"));
		add_dependency(selProj, get_project("engine/renders/render_xbox360"));

		selProj.Object.Save();

		wizard.OkCancelAlert(
            "X-Ray Application Xbox 360 Wizard has been completed successfully!\r\n\r\n" +
...
function AddFilesToCustomProj(proj, project_name, project_path, InfFile) {
    try {
        ...
        if (draw_splash_screen) {
            ...
            wizard.AddSymbol(
		        "PRE_INITIALIZE",
		        '\r\n	xray::engine::preinitialize		( GetCommandLine( ), "' + wizard.FindSymbol('PROJECT_NAME') + '\");'
		    );

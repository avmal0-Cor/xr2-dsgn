...
#include <sys/types.h> /* reffer definition of size_t and sys_addr_t */
#include <sys/integertypes.h> /* reffer definition of uint64_t */
...
_NO_RETURN(exitspawn(const char *, char *const [], char *const [], sys_addr_t, size_t, int, uint64_t)); /* only PS3 */
_NO_RETURN(_Exitspawn(const char *, char *const [], char *const [], sys_addr_t, size_t, int, uint64_t)); /* only PS3 */
_END_EXTERN_C
...
_END_EXTERN_C
  ...
  #if defined(_HAS_STRICT_LINKAGE) && _HAS_STRICT_LINKAGE
...
using  _CSTD malloc_usable_size;
using  _CSTD malloc_stats;
using  _CSTD malloc_managed_size;
#endif // __PME___

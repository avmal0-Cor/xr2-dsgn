...
//DOM-IGNORE-BEGIN
#ifndef _CELL_LIBSHA256_SPU_H_
#define _CELL_LIBSHA256_SPU_H_
...
#define CELL_SHA256_BUFFQSIZE 32
//DOM-IGNORE-END
...
typedef struct {
	...
	void *qdata;
	int free_flags;
} CellSha256SpuInstance;//*** Note to editor, the contents of this structure should be hidden, in the same way that CellSpurs is ***
//Summary:
//Instance of an SHA-256 SPU task
//
//Description:
//This is the datatype of a SHA-256 SPU instance, and holds information about the SPURS usage for this particular instance.
//It is not necessary or appropriate for the application to handle the members of this datatype directly.



/* Summary
   Create a SHA-256 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha256 :         Pointer to a SHA\-256 SPURS work area.
   ...
   CELL_ERROR_HASH_NULL_POINTER            0x80f00001   <c>taskset</c> or <c>sha256</c>
                                                         is a null pointer.
   ...
   SHA-256 hash value of some data. For <c>sha256</c> and <c>taskset</c>,
   these should be provided to the function and allocated and
   initialized accordingly.
   
   The structure <c>sha256</c> is used to hold information about
   the SHA-256 SPURS task that is created by this function and
   ...
   See Also
   <c>cellSha256SpuEndTask</c>                                                                              */
int cellSha256SpuCreateTask(CellSha256SpuInstance *sha256,
							CellSpursTaskset *taskset,
							CellSpursQueue *queue, 
							CellSpursEventFlag *event, 
							void *context_store,
...
/* Summary
   ...
   If <c>sha256==NULL</c> or <c>data==0</c>, 
   ...
   is returned.
   Description
   This function adds more data to the SHA-256 SPURS task for
   ...
   This flag is used to check for the status of the transfer in <c>cellSha256SpuProcessData</c>.
   Note
   ...
   operation is checked by calling <c>cellSha256SpuCheckStatus</c>.
   See Also
   <c>cellSha256SpuCreateTask</c>, <c>cellSha256SpuCheckStatus</c>                               */
int cellSha256SpuProcessData(CellSha256SpuInstance *sha256,
							 uint64_t data,
							 unsigned int size,
							 uint16_t flag,
							 int wait);
...
/* Summary
   Check status of an SHA-256 process.
   Conditions
   Multi-thread safe.
   Parameters
   sha256 :  Pointer to a SHA\-256 SPURS work area.
   ...
   If <c>sha256==NULL</c>, then <c>CELL_ERROR_HASH_NULL_POINTER</c> is returned.
   ...
   is returned.
   Description
   This function returns the status of an SHA-256 SPURS process
   that was initiated using <c>cellSha256SpuProcessData</c>. If <c>wait</c>
   ...
   See Also
   <c>cellSha256SpuCreateTask</c>, <c>cellSha256SpuProcessData</c>             */
int cellSha256SpuCheckStatus(CellSha256SpuInstance *sha256,
							 uint16_t flag,
							 int wait);

/* Summary
   End a SHA-256 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha256 :  Pointer to a SHA\-256 SPURS work area
   ...
   If <c>sha256==NULL</c> or <c>hash==0</c>, 
   then <c>CELL_ERROR_HASH_NULL_POINTER</c> is returned.
   ...
   Description
   This function ends the SHA-256 SPURS as stored in the <c>sha256</c>
   ...
   ensure all previous data is processed by calling <c>cellSha256SpuCheckStatus</c>
   before calling this function.
   See Also
   <c>cellSha256SpuCreateTask</c>, <c>cellSha256SpuProcessData</c>,
   <c>cellSha256SpuCheckStatus</c>                                                  */
int cellSha256SpuEndTask(CellSha256SpuInstance *sha256,
						 uint64_t hash);

...
//DOM-IGNORE-BEGIN
#ifndef _CELL_LIBSHA512_SPU_H_
#define _CELL_LIBSHA512_SPU_H_
...
#define CELL_SHA512_DIGEST_SIZE		64
#define CELL_SHA512_BUFFQSIZE 32
//DOM-IGNORE-END
...
typedef struct {
	...
	void *qdata;
	int free_flags;
} CellSha512SpuInstance;//*** Note to editor, the contents of this structure should be hidden, in the same way that CellSpurs is ***
//Summary:
//Instance of an SHA-512 SPU task
//
//Description:
//This is the datatype of a SHA-512 SPU instance, and holds information about the SPURS usage for this particular instance.
//It is not necessary or appropriate for the application to handle the members of this datatype directly.


/* Summary
   Create a SHA-512 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha512 :         Pointer to a SHA\-512 SPURS work area.
   ...
   CELL_ERROR_HASH_NULL_POINTER            0x80f00001   <c>taskset</c> or <c>sha512</c>
                                                         is a null pointer.
   ...
   SHA-512 hash value of some data. For <c>sha512</c> and <c>taskset</c>,
   these should be provided to the function and allocated and
   initialized accordingly.
   
   The structure <c>sha512</c> is used to hold information about
   the SHA-512 SPURS task that is created by this function and
   ...
   See Also
   <c>cellSha512SpuEndTask</c>                                                                              */
int cellSha512SpuCreateTask(CellSha512SpuInstance *sha512,
						  CellSpursTaskset *taskset,
						  CellSpursQueue *queue, 
						  CellSpursEventFlag *event, 
						  void *context_store,
...
/* Summary
   ...
   If <c>sha512==NULL</c> or <c>data==0</c>, 
   ...
   is returned.
   Description
   This function adds more data to the SHA-512 SPURS task for
   ...
   This flag is used to check for the status of the transfer in <c>cellSha512SpuProcessData</c>.
   Note
   ...
   operation is checked by calling <c>cellSha512SpuCheckStatus</c>.
   See Also
   <c>cellSha512SpuCreateTask</c>, <c>cellSha512SpuCheckStatus</c>                               */
int cellSha512SpuProcessData(CellSha512SpuInstance *sha512,
						   uint64_t data,
						   unsigned int size,
						   uint16_t flag,
						   int wait);
...
/* Summary
   Check status of an SHA-512 process.
   Conditions
   Multi-thread safe.
   Parameters
   sha512 :  Pointer to a SHA\-512 SPURS work area.
   ...
   If <c>sha512==NULL</c>, then <c>CELL_ERROR_HASH_NULL_POINTER</c> is returned.
   ...
   is returned.
   Description
   This function returns the status of an SHA-512 SPURS process
   that was initiated using <c>cellSha512SpuProcessData</c>. If <c>wait</c>
   ...
   See Also
   <c>cellSha512SpuCreateTask</c>, <c>cellSha512SpuProcessData</c>             */
int cellSha512SpuCheckStatus(CellSha512SpuInstance *sha512,
							  uint16_t flag,
							  int wait);

/* Summary
   End a SHA-512 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha512 :  Pointer to a SHA\-512 SPURS work area
   hash :  Effective address to store the hash value for the data on
           the PPU. This area should be at least 64 bytes in size
           and 16\-byte aligned.
   Returns
   <c>CELL_OK</c>(0) is returned for success.
 
   If <c>sha512==NULL</c> or <c>hash==0</c>, 
   then <c>CELL_ERROR_HASH_NULL_POINTER</c> is returned.
   ...
   Description
   This function ends the SHA-512 SPURS as stored in the <c>sha512</c>
   ...
   ensure all previous data is processed by calling <c>cellSha512SpuCheckStatus</c>
   before calling this function.
   See Also
   <c>cellSha512SpuCreateTask</c>, <c>cellSha512SpuProcessData</c>,
   <c>cellSha512SpuCheckStatus</c>                                                  */
int cellSha512SpuEndTask(CellSha512SpuInstance *sha512,
					   uint64_t hash);

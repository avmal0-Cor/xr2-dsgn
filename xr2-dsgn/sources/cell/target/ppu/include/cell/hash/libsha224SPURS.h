...
//DOM-IGNORE-BEGIN
#ifndef _CELL_LIBSHA224_SPU_H_
#define _CELL_LIBSHA224_SPU_H_
...
#define CELL_SHA224_BUFFQSIZE 32
#define CELL_SHA224_DIGEST_SIZE		28
//DOM-IGNORE-END
...
typedef struct {
	...
	void *qdata;
	int free_flags;
} CellSha224SpuInstance;//*** Note to editor, the contents of this structure should be hidden, in the same way that CellSpurs is ***
//Summary:
//Instance of an SHA-224 SPU task
//
//Description:
//This is the datatype of a SHA-224 SPU instance, and holds information about the SPURS usage for this particular instance.
//It is not necessary or appropriate for the application to handle the members of this datatype directly.

/* Summary
   Create a SHA-224 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha224 :         Pointer to a SHA\-224 SPURS work area.
   ...
   CELL_ERROR_HASH_NULL_POINTER            0x80f00001   <c>taskset</c> or <c>sha224</c>
                                                         is a null pointer.
   ...
   SHA-224 hash value of some data. For <c>sha224</c> and <c>taskset</c>,
   these should be provided to the function and allocated and
   initialized accordingly.
   
   The structure <c>sha224</c> is used to hold information about
   the SHA-224 SPURS task that is created by this function and
   ...
   See Also
   <c>cellSha224SpuEndTask</c>                                                                              */
int cellSha224SpuCreateTask(CellSha224SpuInstance *sha224,
						  CellSpursTaskset *taskset,
						  CellSpursQueue *queue, 
						  CellSpursEventFlag *event, 
						  void *context_store,
...
/* Summary
   ...
   If <c>sha224==NULL</c> or <c>data==0</c>, 
   ...
   is returned.
   Description
   This function adds more data to the SHA-224 SPURS task for
   ...
   This flag is used to check for the status of the transfer in <c>cellSha224SpuProcessData</c>.
   Note
   ...
   operation is checked by calling <c>cellSha224SpuCheckStatus</c>.
   See Also
   <c>cellSha224SpuCreateTask</c>, <c>cellSha224SpuCheckStatus</c>                               */

int cellSha224SpuProcessData(CellSha224SpuInstance *sha224,
						   uint64_t data,
						   unsigned int size,
						   uint16_t flag,
						   int wait);
...
/* Summary
   Check status of an SHA-224 process.
   Conditions
   Multi-thread safe.
   Parameters
   sha224 :  Pointer to a SHA\-224 SPURS work area.
   ...
   If <c>sha224==NULL</c>, then <c>CELL_ERROR_HASH_NULL_POINTER</c> is returned.
   ...
   is returned.
   Description
   This function returns the status of an SHA-224 SPURS process
   that was initiated using <c>cellSha224SpuProcessData</c>. If <c>wait</c>
   ...
   See Also
   <c>cellSha224SpuCreateTask</c>, <c>cellSha224SpuProcessData</c>             */
int cellSha224SpuCheckStatus(CellSha224SpuInstance *sha224,
							  uint16_t flag,
							  int wait);

/* Summary
   End a SHA-224 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha224 :  Pointer to a SHA\-224 SPURS work area
   hash :  Effective address to store the hash value for the data on
           the PPU. This area should be at least 28 bytes in size
           and 16\-byte aligned.
   Returns
   <c>CELL_OK</c>(0) is returned for success.

   If <c>sha224==NULL</c> or <c>hash==0</c>, 
   then <c>CELL_ERROR_HASH_NULL_POINTER</c> is returned.
   ...
   Description
   This function ends the SHA-224 SPURS as stored in the <c>sha224</c>
   ...
   ensure all previous data is processed by calling <c>cellSha224SpuCheckStatus</c>
   before calling this function.
   See Also
   <c>cellSha224SpuCreateTask</c>, <c>cellSha224SpuProcessData</c>,
   <c>cellSha224SpuCheckStatus</c>                                                  */
int cellSha224SpuEndTask(CellSha224SpuInstance *sha224,
					   uint64_t hash);

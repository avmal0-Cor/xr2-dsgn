...
//DOM-IGNORE-BEGIN
#ifndef _CELL_LIBSHA1_SPU_H_
#define _CELL_LIBSHA1_SPU_H_
...
typedef struct {
	...
	void *qdata;
	int free_flags;
} CellSha1SpuInstance;//*** Note to editor, the contents of this structure should be hidden, in the same way that CellSpurs is ***
//Summary:
//Instance of an SHA-1 SPU task
//
//Description:
//This is the datatype of a SHA-1 SPU instance, and holds information about the SPURS usage for this particular instance.
//It is not necessary or appropriate for the application to handle the members of this datatype directly.


/* Summary
   Create a SHA-1 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha1 :         Pointer to a SHA\-1 SPURS work area.
   ...
   CELL_ERROR_HASH_NULL_POINTER            0x80f00001   <c>taskset</c> or <c>sha1</c>
                                                         is a null pointer.
   ...
   SHA-1 hash value of some data. For <c>sha1</c> and <c>taskset</c>,
   these should be provided to the function and allocated and
   initialized accordingly.
   
   The structure <c>sha1</c> is used to hold information about
   the SHA-1 SPURS task that is created by this function and
   ...
   See Also
   <c>cellSha1SpuEndTask</c>                                                                              */
int cellSha1SpuCreateTask(CellSha1SpuInstance *sha1,
						  CellSpursTaskset *taskset,
						  CellSpursQueue *queue, 
						  CellSpursEventFlag *event, 
						  void *context_store,
...
/* Summary
   ...
   If <c>sha1==NULL</c> or <c>data==0</c>, then 
   ...
   is returned.
   Description
   This function adds more data to the SHA-1 SPURS task for
   ...
   This flag is used to check for the status of the transfer in <c>cellSha1SpuProcessData</c>.
   Note
   ...
   operation is checked by calling <c>cellSha1SpuCheckStatus</c>.
   See Also
   <c>cellSha1SpuCreateTask</c>, <c>cellSha1SpuCheckStatus</c>                               */
int cellSha1SpuProcessData(CellSha1SpuInstance *sha1,
						   uint64_t data,
						   unsigned int size,
						   uint16_t flag,
						   int wait);
...
/* Summary
   Check status of an SHA-1 process.
   Conditions
   Multi-thread safe.
   Parameters
   sha1 :  Pointer to a SHA\-1 SPURS work area.
   ...
   If <c>sha1==NULL</c>, then <c>CELL_ERROR_HASH_NULL_POINTER</c> is returned.
   ...
   is returned.
   Description
   This function returns the status of an SHA-1 SPURS process
   that was initiated using <c>cellSha1SpuProcessData</c>. If <c>wait</c>
   ...
   See Also
   <c>cellSha1SpuCreateTask</c>, <c>cellSha1SpuProcessData</c>             */
int cellSha1SpuCheckStatus(CellSha1SpuInstance *sha1,
							  uint16_t flag,
							  int wait);

/* Summary
   End a SHA-1 SPURS task.
   Conditions
   Multi-thread safe.
   Parameters
   sha1 :  Pointer to a SHA\-1 SPURS work area
   hash :  Effective address to store the hash value for the data on
           the PPU. This area should be at least 20 bytes in size
           and 16\-byte aligned.
   Returns
   <c>CELL_OK</c>(0) is returned for success.

   If <c>sha1==NULL</c> or <c>hash==0</c>, 
   ...
   Description
   This function ends the SHA-1 SPURS as stored in the <c>sha1</c>
   ...
   ensure all previous data is processed by calling <c>cellSha1SpuCheckStatus</c>
   before calling this function.
   See Also
   <c>cellSha1SpuCreateTask</c>, <c>cellSha1SpuProcessData</c>,
   <c>cellSha1SpuCheckStatus</c>                                                  */
int cellSha1SpuEndTask(CellSha1SpuInstance *sha1,
					   uint64_t hash);

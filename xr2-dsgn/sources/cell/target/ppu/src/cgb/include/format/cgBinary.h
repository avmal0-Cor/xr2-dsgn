...
#if defined(CGBINARY_FORMAT_ENABLE_REVISION7)
#define CG_BINARY_FORMAT_REVISION   0x00000007
#else
...
struct CgBinaryVertexProgram
{
    ...
    unsigned int  attributeInputMask;  // attributes vs reads from (uses SET_VERTEX_ATTRIB_INPUT_MASK bits)
    unsigned int  attributeOutputMask; // attributes vs writes to  (uses SET_VERTEX_ATTRIB_OUTPUT_MASK bits)
    unsigned int  userClipMask;        // user clip plane enables  (uses SET_USER_CLIP_PLANE_CONTROL bits)
#if defined(CGBINARY_FORMAT_ENABLE_REVISION7)
    unsigned short texUnitUsed;        // texid 0..15 use one bit each, bit n is 1 if texunit n is used
#endif
};
...
#if defined(CGBINARY_FORMAT_ENABLE_REVISION7)
    unsigned short noParanoidTexFetch;  // texid 0..15 use one bit each marking whether the texture unit needs to do paranoid fetches (bit set to 1 means it doesn't need to) (used for SET_NO_PARANOID_TEXTURE_FETCHES)
    unsigned short texUnitUsed;         // texid 0..15 use one bit each, bit n is 1 if texunit n is used
#endif
...
struct CgBinaryProgram
{
    ...
    // value from CGprofile or CgBinProfileEnum
    CGprofile profile;
...
#if defined(CGBINARY_FORMAT_ENABLE_REVISION7)
    // source filename, 0 if unspecified
    CgBinaryOffset  filename;
#endif
...
2007-08-01
Revision 7: Added filename, noParanoidTexFetch and texUnitUsed.

...
/* The PPU/SPU shared segments should be in 0x3000_0000 - 0x3fff_ffff */

__ph_text_srodata_start = 0x30000000;  /* start address of PPU/SPU shared RO segment */
__ph_sdata_start        = 0x30010000;  /* start address of PPU/SPU shared RW segment */

/* The PPU/SPU local segments should be in 0x4000_0000 - 0x4fff_ffff */

__ph_rodata_start       = 0x40000000;  /* start address of PPU local RO segment */
__ph_data_start         = 0x40010000;  /* start address of PPU local RW segment */
...
PHDRS
{
  ...
  ph_proc_prx_param 0x60000002      FLAGS(0x0); /* PT_PROC_PRX   (Cell OS Lv-2 Specific segment) *//* PRX */
}
SECTIONS
{
  /* Read-only sections, merged into text segment: */
  PROVIDE (__executable_start = __ph_text_srodata_start); . = __ph_text_srodata_start + SIZEOF_HEADERS;
  ...
  .sys_proc_param : { *(.sys_proc_param) } :ph_text_srodata :ph_proc_param
  .sys_proc_prx_param : { KEEP (*(.sys_proc_prx_param)) } :ph_text_srodata :ph_proc_prx_param	/* PRX */

  /* Here is a boundary of page to separate the PPU/SPU RW region
     from the PPU/SPU RO region */
  . = __ph_sdata_start;
  . = ALIGN(64K);

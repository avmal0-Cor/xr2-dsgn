...
//
//     Computes both the sine and cosine of the each of 
//     two double slots.
//
_FUNC_DEF(void, sincosd2, (vec_double2 x, vec_double2* s, vec_double2* c))
{
    ...
    //   sx = x + xl3 * (((((s0 * xl2 + s1) * xl2 + s2) * xl2 + s3) * xl2 + s4) * xl2 + s5)
    //
    ...
    *s = spu_sel((vec_double2)spu_xor(spu_splats(0x8000000000000000ull),(vec_ullong2)*s),*s,sinMask);
    *c = spu_sel((vec_double2)spu_xor(spu_splats(0x8000000000000000ull),(vec_ullong2)*c),*c,cosMask);

    
    // if input = +/-Inf return NAN
    //
    *s = spu_sel(*s, nan, cmpinfd2(x));
    *c = spu_sel(*c, nan, cmpinfd2(x));

    // if input = 0 or denorm return input (for sin) or 1.0 (for cos)
    //
    vec_ullong2 zeroMask = cmpzerodenormd2(x);
    *s = spu_sel(*s,x,zeroMask);
    *c = spu_sel(*c,spu_splats((double)1.0L),zeroMask);
}

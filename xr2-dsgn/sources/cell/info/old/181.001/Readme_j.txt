...
パッケージには、以下のコンポーネントは含まれません。
  - COLLADA_Pipeline
  - SCREAM Runtime Library for SDK181 (近日中にリリース致します)

infoディレクトリ内に以下のものが含まれますので、必ず参照してください。
なお、今回、新規機能及び変更点はありませんでしたので、Important_Changes_*の
ドキュメントは含めていません。
  ...
  - text/ 以下: テキスト版のNotice、Bugfix

(*) 内容の変更がありませんので、SDK180のドキュメントと同じファイルと
    なります。
...
提供している以下のパッケージを必ずダウンロードしてください。
     ...
     PS3_Toolchain-Win_180_002.zip     gcc/gdb Windows版
     PS3_Toolchain-Linux_180_002.zip   gcc/gdb Linux版
     PS3_SDKDoc-180_01-Latest-*.zip    ドキュメント
...
- SDK100以前の環境から直接SDK181へアップデートした場合の動作についての
  保障はいたしませんので、ご注意ください。

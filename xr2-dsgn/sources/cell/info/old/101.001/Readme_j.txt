...
PLAYSTATION(R)3 Programmer Tool Runtime Library 101.001
...
infoディレクトリ内に以下のものが含まれますので、必ず参照してください。
  ...
  - ソフトウェアセットアップガイド (*)
  - Important_Changes： SDK101の新規機能、変更点、注意事項
  - Notice: 既知の不具合、制限事項
  - Bugfix: 不具合修正
  - PLAYSTATION(R)3 FAQ  (*)
  - Referenceシステム 活用ガイド (*)
...
(*) 内容の変更がありませんので、SDK100のドキュメントと同じファイルと
    なります。

このパッケージに含まれるソフトウェアをご使用の場合、別パッケージで
提供している以下のパッケージを必ずダウンロードしてください。
今回、SDKパッケージ、ドキュメントパッケージ以外の更新は行っていません。
Reference Tool CP update fileパッケージ、Toolchainのパッケージは、
SDK094のものを、Performance Analyzer ToolsパッケージはSDK093でリリース
したものをご使用ください。

     PS3_CP_Update-094_002.zip         Reference Tool CP update file
     PS3_SDK_SampleData-101_001.zip    sample data 
     PS3_Toolchain-Win_094_002.zip     gcc/gdb Windows版
     PS3_Toolchain-Linux_094_002.zip   gcc/gdb Linux版
     PS3_SDKDoc-101_01-Latest-*.zip    ドキュメント
     PS3_SDKDoc-101_01-(patch_from_100_01)-*.zip  パッチドキュメント
...
|---target
|    |---bootrom            Flash Memoryイメージファイル(ebootrom)
